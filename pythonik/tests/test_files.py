...
from pythonik.specs.files import (
    ...
    GET_ASSET_PROXY_PATH_BY_PROXY_ID,
...
def test_get_proxy_by_proxy_id():
    with requests_mock.Mocker() as m:
        ...
        mock_address = FilesSpec.gen_url(
            GET_ASSET_PROXY_PATH_BY_PROXY_ID.format(asset_id, proxy_id)
        )

        m.get(mock_address, json=data)
        client = PythonikClient(app_id=app_id, auth_token=auth_token, timeout=3)
        client.files().get_asset_proxy_by_proxy_id(asset_id, proxy_id)
...
def test_get_proxy():
    with requests_mock.Mocker() as m:
        ...
        model = Proxy()
        data = model.model_dump()
        mock_address = FilesSpec.gen_url(GET_ASSET_PROXY_PATH.format(asset_id))

        m.get(mock_address, json=data)
        client = PythonikClient(app_id=app_id, auth_token=auth_token, timeout=3)
        client.files().get_asset_proxy(asset_id)
